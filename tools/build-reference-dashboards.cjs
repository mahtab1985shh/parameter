...
doc=doc.replace(/<script src="https:\/\/cdnjs.cloudflare.com\/ajax\/libs\/Chart.js\/4.4.1\/chart.umd.min.js"><\/script>/,()=>'<script>'+fs.readFileSync(path.join(dir,'reference-chart.umd.js'),'utf8').replace(/<\/script/gi,'<\\/script')+'</script>');
...
const css='#view-dashboard.reference-dashboard-active{height:auto!important;overflow:visible!important;min-width:0}.reference-dashboard-frame{width:100%;height:calc(100dvh - 210px);min-height:650px;border:0;display:block;background:var(--surface)}.reference-dashboard-active .dash-mode-switch{flex-wrap:wrap}.reference-dashboard-active .dashboard-modebar{height:auto!important}@media(max-width:700px){.reference-dashboard-frame{height:calc(100dvh - 190px);min-height:580px}}\n';
let h=fs.readFileSync(file,'utf8'),match=h.match(/var binary=atob\("([^"]+)"\)/),p=Buffer.from(match[1],'base64').toString('utf8');
if(p.includes('/* REFERENCE DASHBOARDS START */'))throw Error('Already installed');
const enc=s=>JSON.stringify(s).slice(1,-1);
if(!p.includes('function userUtilityAction(type){'))throw Error('Missing insertion point');
p=p.replace('function userUtilityAction(type){',()=>enc(js)+'function userUtilityAction(type){');
p=p.replace('.parameter-date-overlay{',()=>enc(css)+'.parameter-date-overlay{');
fs.writeFileSync(file,h.replace(match[1],Buffer.from(p).toString('base64')));
console.log('Added two reference dashboard tabs with bundled charts.');
